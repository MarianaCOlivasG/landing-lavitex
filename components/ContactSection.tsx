  ...
  return (
    <section id="contact" className="w-full py-section-gap bg-gradient-to-b from-slate-100 to-slate-200 border-t border-slate-200">
      <div className="max-w-container-max mx-auto px-gutter md:px-16 lg:px-24 grid grid-cols-1 lg:grid-cols-2 gap-16">
        {/* Contact Form */}
        <div className="bg-white/95 backdrop-blur-md p-10 rounded-DEFAULT shadow-md border border-white">
          <div className="mb-8">
          ...
          <form className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input className="w-full border-0 border-b-2 border-slate-200 focus:border-cyan-600 focus:ring-0 px-0 py-3 text-slate-900 font-body-md text-body-md transition-colors bg-transparent" placeholder="Juan Pérez" type="text" />
              </div>
              <div>
                <label className="block font-label-caps text-label-caps text-slate-500 mb-2 uppercase">Número de Teléfono</label>
                <input className="w-full border-0 border-b-2 border-slate-200 focus:border-cyan-600 focus:ring-0 px-0 py-3 text-slate-900 font-body-md text-body-md transition-colors bg-transparent" placeholder="+52 (55) 1234-5678" type="tel" />
              </div>
            </div>
            <div>
              <label className="block font-label-caps text-label-caps text-slate-500 mb-2 uppercase">Correo</label>
              <input className="w-full border-0 border-b-2 border-slate-200 focus:border-cyan-600 focus:ring-0 px-0 py-3 text-slate-900 font-body-md text-body-md transition-colors bg-transparent" placeholder="ejemplo@correo.com" type="email" />
            </div>
            <div>
              <label className="block font-label-caps text-label-caps text-slate-500 mb-2 uppercase">Mensaje</label>
              <textarea className="w-full border-0 border-b-2 border-slate-200 focus:border-cyan-600 focus:ring-0 px-0 py-3 text-slate-900 font-body-md text-body-md transition-colors bg-transparent resize-none" placeholder="Describe tus necesidades de textiles comerciales..." rows={4}></textarea>
            </div>
            <button className="bg-gradient-to-r from-cyan-600 to-cyan-500 text-white px-8 py-4 rounded-DEFAULT font-headline-sm text-headline-sm hover:from-cyan-500 hover:to-cyan-400 hover:-translate-y-0.5 transition-all duration-300 w-full md:w-auto flex items-center justify-center gap-2 shadow-md" type="submit">
              Enviar Consulta
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
            <div className="flex items-start gap-4">
              <span className="material-symbols-outlined text-cyan-600 mt-1" data-icon="map">map</span>
              <div>
            ...
            <div className="flex items-start gap-4">
              <span className="material-symbols-outlined text-cyan-600 mt-1" data-icon="support_agent">support_agent</span>
              <div>
